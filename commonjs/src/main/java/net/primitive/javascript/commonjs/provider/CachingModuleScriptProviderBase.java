...
public abstract class CachingModuleScriptProviderBase implements ModuleScriptProvider, Serializable {
	...
	public ModuleScript getModuleScript(Scope scope, String moduleId, URI moduleUri, Scriptable paths) throws Exception {
		final CachedModuleScript cachedModule1 = getLoadedModule(moduleId);
		final Object validator1 = getValidator(cachedModule1);
		final ModuleSource moduleSource = (moduleUri == null) ? moduleSourceProvider.loadSource(moduleId, paths, validator1) : moduleSourceProvider.loadSource(moduleUri,
		        validator1);
		...
		try {
			...
			synchronized (loadLocks[(idHash >>> loadLockShift) & loadLockMask]) {
				...
				final ModuleScript moduleScript = loadModuleScript(moduleSource, reader, sourceUri);
	...
	protected abstract ModuleScript loadModuleScript(final ModuleSource moduleSource, final Reader reader, final URI sourceUri);
